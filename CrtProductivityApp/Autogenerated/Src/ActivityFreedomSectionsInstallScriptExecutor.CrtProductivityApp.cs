	...
	internal class ActivityFreedomSectionsInstallScriptExecutor : IInstallScriptExecutor
	{
		...
		private void AddCalendarAndTaskSectionsToWorkplaces(UserConnection uc, List<Guid> workplaceIds) {
			workplaceIds.ForEach(workplaceId => {
				AddSectionToWorkplace(uc, workplaceId, _calendarSysModuleId);
				AddSectionToWorkplace(uc, workplaceId, _tasksSysModuleId);
			});
		}

		private void AddSectionToWorkplace(UserConnection uc, Guid workplaceId, Guid sectionId) {
			new Insert(uc)
